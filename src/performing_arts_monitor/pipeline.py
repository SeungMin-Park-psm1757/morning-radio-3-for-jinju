...
OUTPUT_CATEGORY_SET = set(OUTPUT_CATEGORY_ORDER)
AUDITION_WHEN_HINTS = (
    "접수",
    "지원",
    "마감",
...
DATE_PATTERN = re.compile(
    r"(?:\d{4}[./-]\s*\d{1,2}[./-]\s*\d{1,2}(?:\([^)]{0,8}\))?(?:\s*(?:오전|오후)?\s*\d{1,2}:\d{2})?"
    r"|\d{1,2}월\s*\d{1,2}일(?:\([^)]{0,8}\))?(?:\s*(?:오전|오후)?\s*\d{1,2}:\d{2})?"
    r"|\d{1,2}/\d{1,2}(?:\s*(?:오전|오후)?\s*\d{1,2}:\d{2})?)"
)
EMAIL_PATTERN = re.compile(r"\b[0-9A-Za-z._%+-]+@[0-9A-Za-z.-]+\.[A-Za-z]{2,}\b")
CHILD_ROLE_EXCLUDE_TERMS = (
    "아역",
    "어린이",
    "키즈",
    "주니어",
...
AUDITION_STRUCTURED_DETAIL_TERMS = (
    "지원 자격",
    "지원자격",
    "모집 요강",
    "모집요강",
    ...
    if category not in OUTPUT_CATEGORY_SET:
        ...
        if not exclude_reason:
            if category == "people":
                ...
                exclude_reason = "현재 모니터 초점과 맞지 않음"
...
def _local_assessment(
    ...
    audition_profile = _audition_profile_signals(text)
    ...
    if category == "audition":
        if audition_profile["female_hits"]:
            profile_boost += min(4.5 + max(audition_profile["female_hits"] - 1, 0) * 1.0, 7.0)
        if audition_profile["seoul_hits"]:
            profile_boost += min(3.0 + max(audition_profile["seoul_hits"] - 1, 0) * 0.5, 4.0)
        if audition_profile["structured_hits"]:
            profile_boost += min(audition_profile["structured_hits"] * 0.8, 2.5)
        if audition_profile["non_seoul_hits"] and not audition_profile["seoul_hits"]:
            profile_boost -= 2.5
        if audition_profile["male_hits"]:
            profile_boost -= 5.0
        if audition_profile["child_hits"]:
            profile_boost -= 12.0
        if audition_profile["genre_exclude_hits"]:
            profile_boost -= 10.0
...
def _select_items(config: AppConfig, triaged_items: list[TriagedItem]) -> list[TriagedItem]:
    strict_candidates = [
        ...
        if item.keep and item.final_score >= config.score_threshold and item.category in OUTPUT_CATEGORY_SET
    ]
    strict_candidates.sort(key=lambda item: (item.final_score, item.published_at), reverse=True)

    by_category = {key: [] for key in OUTPUT_CATEGORY_ORDER}
    ...
    if selected:
        return sorted(selected, key=lambda item: (OUTPUT_CATEGORY_ORDER.index(item.category), -item.final_score))
    ...
    fallback_candidates = [
        ...
        and item.category in {"audition", "support", "works_casting", "company_news"}
    ...
    return sorted(fallback_selected, key=lambda item: (OUTPUT_CATEGORY_ORDER.index(item.category), -item.final_score))
...
def _select_news_items(config: AppConfig, triaged_items: list[TriagedItem]) -> list[TriagedItem]:
    strict_candidates = [
        ...
        and item.category in OUTPUT_CATEGORY_SET
        and ("티켓오픈" not in item.secondary_tags or bool(item.mentioned_people))
    ]
...
def _render_message_digest(
    ...
    for section in digest.sections:
        ...
        for item in section.items:
            ...
            for bullet in item.detail_bullets:
...
def _detail_bullets(item: CollectedItem, category: str) -> list[str]:
    if category != "audition":
        return []
    return _audition_detail_bullets(item)


def _audition_profile_signals(text: str) -> dict[str, int]:
    return {
        "child_hits": _count_hits(text, CHILD_ROLE_EXCLUDE_TERMS),
        "female_hits": _count_hits(text, FEMALE_ROLE_PRIORITY_TERMS),
        "male_hits": _count_hits(text, MALE_ROLE_EXCLUDE_TERMS),
        "seoul_hits": _count_hits(text, SEOUL_PRIORITY_TERMS),
        "non_seoul_hits": _count_hits(text, NON_SEOUL_LOCATION_TERMS),
        "structured_hits": _count_hits(text, AUDITION_STRUCTURED_DETAIL_TERMS),
        "genre_exclude_hits": _count_hits(text, AUDITION_GENRE_EXCLUDE_TERMS),
    }
...
def _audition_detail_bullets(item: CollectedItem) -> list[str]:
    ...
    when_text = _extract_audition_fact(
        ...
        AUDITION_WHEN_HINTS,
        require_date=True,
        fallback="원문에서 접수·실기 일정을 확인해 주세요.",
    )
    where_text = _extract_audition_where(item, combined_text)
    condition_text = _extract_audition_fact(
        combined_text,
        AUDITION_CONDITION_HINTS,
        fallback="지원 자격과 제출물은 원문 공고 확인이 필요합니다.",
    )
    return [
        f"언제: {when_text}",
        f"어디서: {where_text}",
...
def _extract_audition_where(item: CollectedItem, text: str) -> str:
    email_match = EMAIL_PATTERN.search(text)
    if email_match:
        return f"이메일 접수 ({email_match.group(0)})"

    where_text = _extract_audition_fact(
        text,
        AUDITION_WHERE_HINTS,
        fallback="원문 공고 링크에서 장소나 접수처를 확인해 주세요.",
    )
    if "원문 공고 링크" not in where_text:
        return where_text

    link_hint = _audition_link_hint(item)
    if link_hint:
        return f"온라인 공고 확인 ({link_hint})"
    return where_text


def _audition_link_hint(item: CollectedItem) -> str:
    for url in [*item.external_urls, item.url]:
        lowered = url.lower()
...
def _extract_audition_fact(
    ...
    *,
    ...
    candidates = _candidate_fragments(text, hints)
    scored: list[tuple[int, int, int, str]] = []
    for index, candidate in enumerate(candidates):
        hint_hits = sum(1 for hint in hints if hint.lower() in candidate.lower())
        if hint_hits == 0:
            continue
        score = hint_hits * 3
        if DATE_PATTERN.search(candidate):
            score += 4
        if require_date and not DATE_PATTERN.search(candidate):
            continue
        if len(candidate) > 120:
            score -= 1
        if score > 0:
            scored.append((score, -len(candidate), -index, candidate))
    if not scored:
        return fallback
...
def _candidate_fragments(text: str, hints: tuple[str, ...]) -> list[str]:
    normalized = _normalize_whitespace(text)
    if not normalized:
        return []

    fragments: list[str] = []
    for part in re.split(r"(?<=[.!?])\s+|[•·▪◆▶※]+|\s+\|\s+|\s{2,}", normalized):
        cleaned = _clean_fact_fragment(part)
        if cleaned:
            fragments.append(cleaned)

    for hint in hints:
        for match in re.finditer(re.escape(hint), normalized, flags=re.IGNORECASE):
            cleaned = _clean_fact_fragment(_sentence_context(normalized, match.start(), match.end()))
            if cleaned:
                fragments.append(cleaned)
...
def _sentence_context(text: str, start_index: int, end_index: int) -> str:
    sentence_start = 0
    for marker in ".?!":
        found = text.rfind(marker, 0, start_index)
        if found >= 0:
            sentence_start = max(sentence_start, found + 1)

    sentence_end = len(text)
    candidates = [text.find(marker, end_index) for marker in ".?!"]
    candidates = [candidate for candidate in candidates if candidate >= 0]
    if candidates:
        sentence_end = min(candidates) + 1

    window = text[sentence_start:sentence_end].strip()
    if window:
        return window

    start = max(0, start_index - 28)
...
def _clean_fact_fragment(text: str) -> str:
    ...
    cleaned = re.sub(r"^(제목|요약|본문|공지|안내)\s*:\s*", "", cleaned)
    cleaned = re.sub(r"https?://\S+", "", cleaned)
    cleaned = re.sub(r"\bgle/\S*", "", cleaned)
    cleaned = re.sub(r"\b(?:www\.|forms\.gle/|docs\.google\.com/forms/\S+|[A-Za-z0-9.-]+\.[A-Za-z]{2,}/)\S*", "", cleaned)
    cleaned = re.sub(r"\bDate\b\s*\d{4}[-./]\d{1,2}[-./]\d{1,2}(?:\s+\d{1,2}:\d{2})?", "", cleaned, flags=re.IGNORECASE)
    if any(marker in cleaned.lower() for marker in ("author", "views", "like", "unlike", "print", "list")):
        return ""
    cleaned = re.sub(r"\s*(자세히 보기|더보기|원문 보기)$", "", cleaned)
    if len(cleaned) < 8:
        return ""
    if len(cleaned) > 110:
        cleaned = cleaned[:109].rstrip() + "…"
    return cleaned
...
def _heuristic_summary(item: CollectedItem) -> str:
    ...
    if "개막" in compact_title or "초연" in compact_title or "프리뷰" in compact_title:
        if title_work_titles:
            return f"{title_work_titles[0]}의 개막 일정이나 주요 캐스팅 변화를 담은 공지입니다."
        return "개막 일정이나 주요 캐스팅 변화를 담은 공지입니다."
...
def _write_run_archive_page(run_dir: Path, digest: DigestRun, timezone: ZoneInfo) -> None:
    sections: list[str] = [
        ...
        ".detail-list{margin:8px 0 0 0;padding-left:18px;color:#344054;}",
    ...
    for section in digest.sections:
        ...
        for item in section.items:
            ...
            if item.detail_bullets:
                sections.append("<ul class='detail-list'>")
                for bullet in item.detail_bullets:
                    sections.append(f"<li>{html.escape(bullet)}</li>")
                sections.append("</ul>")
