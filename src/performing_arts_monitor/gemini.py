...
class GeminiTriage:
    ...
    def _triage_batch(self, items: list[CollectedItem]) -> dict[str, Any]:
        ...
        system_instruction = (
            ...
            "This monitor is optimized for audition opportunities, support programs, and opening or casting developments. "
...
- Actor interview, award, or personality coverage should usually be excluded unless it directly changes an audition opportunity, casting, or opening schedule.
- Child-role-only auditions should usually be excluded for this monitor.
- Prefer adult female-role notices and Seoul-area auditions for this monitor, and exclude audition notices when the metadata does not show a female-role or female-recruiting signal.
- Exclude audition notices when male roles are included alongside the female roles, because this monitor is intentionally narrowed to women-only opportunities.
- Exclude K-pop, idol, dance-team, or model-recruitment notices unless the metadata clearly shows a stage-actor or musical-theatre context that overrides the generic label.
